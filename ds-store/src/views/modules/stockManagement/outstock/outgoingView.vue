<template>
  <el-dialog
    ...
    <el-form ref="outgoing-form" class="form-show" label-width="110px">
      ...
      <el-form-item label="收货单位">
        <span class="itemInput">
          {{ outgoing.outType === "ALLOT" ? outgoing.targetStoreName :
             outgoing.outType === "REFUND" ? '企业总部' : '--'}}
        </span>
        <!-- <span class="itemInput"  v-if="outgoing.outType !== 'REFUND'">{{outgoing.targetStoreName || '--'}}</span>
        <span class="itemInput" v-else>企业总部</span> -->
      </el-form-item>
      <!-- <el-form-item label="物流公司" v-show="flag == 'view'">
        <span class="itemInput">{{outgoing.logisticsCompany || '--'}}</span>
      </el-form-item>
      <el-form-item label="物流单号" v-show="flag == 'view'">
    ...
    <!-- <div class="toptitle">
      ...
      <span class="tip" v-if="storeRoleName === '店长'">合计成本价：{{priceNum(outgoing.costPriceAmount) || '--'}}</span>
      <span class="tip">合计零售价：{{priceNum(outgoing.retailPriceAmount) || '--'}}</span>
    </div> -->
    <div v-if="flag === 'out'" class="logistic">
    ...
    <span slot="footer" class="dialog-footer" >
      ...
      <!-- <el-checkbox style="margin-left: 20px;" v-model="showWholesalePrice" label="显示配送价"></el-checkbox> -->
    </span>
...
export default {
  ...
  methods: {
    ...
    goPrint() {
      if (this.flag === 'out') {
        this.dataFormSubmit(() => {
          document.getElementById('store-outstock').style = 'display: block;'
          printJS({printable: 'store-outstock',  type: 'html', css: './static/print.css'})
          document.getElementById('store-outstock').style = 'display: none;'
        })
      } else {
        document.getElementById('store-outstock').style = 'display: block;'
        printJS({printable: 'store-outstock',  type: 'html', css: './static/print.css'})
