<template>
  <div class="mod-store inventory">
    <el-card class="mode-store-table" style="padding-bottom: 15px;">
      <el-form :model="inventory" :inline="true" class="mode-form-line1" ref="inventory-form" :rules="dataFormRules">
...
import gtSelectGoods from './component/select-goods-modal'
...
export default {
  ...
  data(){
    const validateGoods = (rule, value, callback) => {
      if (this.inventory.goodsList.length === 0) {
        callback(new Error('请选择盘点商品'))
      } else {
        callback()
      }
    }
  ...
  created(){
    ...
    this.id = id
    if (id && this.isAuth('store:checkStock:detail')) {
      // 获取盘点单详情
      this.getDetail(id)
    }
    this.detail()
    ...
    async selectGoods() {
      ...
      if (data.code === 0) {
        if (data.data.total === 1) {
          ...
          this.$nextTick(() => {
            this.$refs.selectgoods.init(this.inventory.goodsList)
          })
        }
      } else {
        this.$message.error(data.msg)
      ...
      if(this.inventory.goodsList&&this.inventory.goodsList.length>0){
        this.$refs['inventory-form'].validate(async (valid) => {
          if (valid && !this.checkInfo()) {
            ...
            const { data } = this.id ? await api.updateInventory(form) : await api.createInventory(form)
            if(data.code == 0){
              this.$router.push({name: 'stockManagement-inventoryManagement'})
            } else if (data.code == -2) {
