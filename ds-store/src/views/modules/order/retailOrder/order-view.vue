<template>
  <section class="order" v-loading="$store.state.loading.global">
    ...
    <el-card style="margin-bottom: 10px;padding-bottom: 10px;">
      ...
      <el-table
        ...
        <el-table-column
          ...
          label="实付小计">
          <template slot-scope="scope">
            {{scope.row.payableAmount ? priceNum(scope.row.payableAmount) : 0}}
          </template>
        </el-table-column>
        <el-table-column
          ...
          label="销售成本">
          <template slot-scope="scope">
            {{scope.row.costPrice ? priceNum(scope.row.costPrice) : 0}}
          </template>
        </el-table-column>
        <el-table-column
          v-if="storeRoleName === '店长' && hasAuthorization"
          ...
          label="成本小计">
          <template slot-scope="scope">
            {{scope.row.costAmount ? priceNum(scope.row.costAmount) : 0}}
          </template>
        </el-table-column>
        <el-table-column
          v-if="storeRoleName === '店长' && hasAuthorization"
          ...
          label="毛利">
          <template slot-scope="scope">
            {{NP.minus(priceNum(scope.row.payableAmount),priceNum(scope.row.costAmount)) || 0}}
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    <el-card style="margin-bottom: 10px;">
      ...
      <el-form-item label="订单成本" v-if="storeRoleName === '店长' && hasAuthorization">
        <span class="itemInput">{{priceNum(baseInfo.costAmount)}}</span>
      </el-form-item>
      <el-form-item label="订单毛利" v-if="storeRoleName === '店长' && hasAuthorization">
        <span class="itemInput" >{{priceNum(baseInfo.payableAmount-baseInfo.costAmount)}}</span>
      </el-form-item>
    </el-card>
