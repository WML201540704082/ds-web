    ...
    <el-card style="margin-bottom: 10px;padding-bottom: 10px;">
      ...
      <el-table
        ...
        <el-table-column
          ...
          label="商品类别">
          <template slot-scope="scope">
            {{scope.row.productCategory ? filterStatus(scope.row.productCategory,productHaveBulkType) : '--'}}
          </template>
        </el-table-column>
        <el-table-column
          prop="productOwner"
...
import { paymentOfMode,orderOfMode, orderType,productHaveBulkType,exchangeStatus, productOwner,exchangeType } from '@/utils/dict'
...
export default {
  ...
  async created() {
    ...
    if (id) {
      ...
      this.baseInfo = {
        ...data,
        createTime: moment(data.integralOrder.createTime).format('YYYY-MM-DD HH:mm:ss')
      }
    }
  },
}
