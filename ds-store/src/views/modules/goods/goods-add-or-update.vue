        ...
        <el-form v-loading="loadingData" ref="goods-form" :rules="dataFormRules" label-width="110px" :model="goods">
          ...
          <el-card style="margin-top: 10px" class="com-goods-edit">
            <el-form-item label="启用状态" prop="saleStatus">
              <el-switch
                :disabled="storeType == 'NEW_RETAIL_AND_DRINK' || storeType == 'NEW_RETAIL'"
            ...
            <el-form-item label="商品类别" prop="type">
              <el-radio-group v-if="id ? false: true" v-model="goods.type">
                <el-radio v-for="item in ((storeType == 'NEW_RETAIL_AND_DRINK' || storeType == 'NEW_RETAIL') ? productNoServiceType : productType)" :key="item.value" :label="item.value">
                  {{item.label}}
                </el-radio>
              </el-radio-group>
            ...
            <el-form-item label="商品名称" class="gt-input-tip large" prop="name">
              <el-input v-model="goods.name" :maxlength="30"><span slot="prefix" >*名称限制30个汉字以内</span></el-input>
            </el-form-item>
            <!-- 零售类商品 -->
            <div v-if="goods.type == 'ENTITY' || goods.type == 'BULK'">
              ...
              <el-form-item label="单位" prop="packingType" v-if="goods.type == 'ENTITY' && (storeType == 'NEW_RETAIL_AND_DRINK' || storeType == 'NEW_RETAIL')">
                  <el-input v-model="goods.packingType" :maxlength="50" placeholder="请输入商品最小销售单位"></el-input>
               </el-form-item>
            </div>

            <el-form-item label="成本价" v-if="storeRoleName === '店长' && hasAuthorization" class="gt-input-tip large" prop="costPrice" :rules="[{required: goods.type == 'ENTITY', message:'请填写商品成本价'}, {validator: validateCost, trigger: 'blur' }]">
              <el-input v-model="goods.costPrice" :placeholder="tip">
                <span slot="prefix" ><span class="unit">元</span>*商品的进货价</span>
              </el-input>
            </el-form-item>

            <el-form-item label="零售价" class="gt-input-tip large" prop="retailPrice">
              <el-input v-model="goods.retailPrice" :placeholder="tip">
                <span slot="prefix" ><span class="unit">元</span>*商品针对普通消费者的实际销售价格</span>
              </el-input>
            </el-form-item>

            <el-form-item label="卡扣技师提成" class="gt-input-tip service-commission-label" v-show="goods.type != 'ENTITY' && goods.type != 'BULK'" prop="serviceCommission">
              <el-input v-model="goods.serviceCommission" placeholder="请填写提成金额">
                <span slot="prefix" ><span class="unit">元</span>*以礼品卡的方式消费时，技师的提成金额</span>
              </el-input>
            </el-form-item>
            ...
            <div class="more-detail-control" @click="changeDetail">
              ...
              <div class="middle-info theme-color">完善更多商品信息 <icon-svg class="down-arrow" :class="{'up': showDetail}" name="double-arrow"></icon-svg></div>
              <div class="base-line"></div>
            </div>
            <div v-show="showDetail">
            <el-form-item label="销售提成" prop="commissionType"  v-if="storeRoleName === '店长' && goods.type != 'BULK'">
              ...
              <el-form-item class="gt-input-tip" prop="commission">
                <el-input v-model="goods.commission" placeholder="请填写提成数值">
                  <span slot="prefix" ><span class="unit">{{goods.commissionType === 'RATIO' ? '%' : '元'}}</span></span>
                </el-input>
              </el-form-item>
            </el-form-item>
            <el-form-item v-if="integral.productStatus && integral.productIntegralType === 'PRODUCT'" label="商品积分" prop="integralPlanId" class="inline-input">
                  ...
                  <div class="sku-edit spec-table" v-show="memberLevelList && memberLevelList.length > 0">
                    <el-table
                      ...
                      <el-table-column
                        ...
                        <template slot-scope="scope">
                          <el-input v-if="goods.memberSwitch" v-model="scope.row[item.vipId]"/>
                          <span v-else >{{scope.row[item.vipId]}}</span>
                        </template>
                      </el-table-column>
                    </el-table>
                  </div>
...
import { companyLevel, companyType, companyStatus, productType,productNoServiceType, saleChannel, saleOnlyOneChannel, timeInterval, memberPriceType,isShowMarketing } from '@/utils/dict'
...
import { isMobile, validatenull, priceRange, isName, limitInputName, twoDecimal } from '@/utils/validate'
...
export default {
  ...
  data() {
    ...
    const validatePrice = (rule, value, callback) => {
      if (value) {
        if (this.goods.commissionType === 'RATIO' && !priceRange(value, 100, 0)) {
          callback(new Error('请输入0-100数字，最多保留小数点后两位'))
        } else if(this.goods.commissionType === 'FIXED_AMOUNT' && !priceRange(value, 1000, 0)) {
    ...
    const validateCost = (rule, value, callback) => {
      if (value) {
        if (!priceRange(value, 499999.99, 0)) {
          callback(new Error('请输入0.00-499999.99的价格数字，最多保留小数点后两位'))
        } else {
          callback()
        }
      } else {
    ...
    return {
      ...
      dataFormRules:{
        ...
        name:[{required: true, message: '请填写商品名称'}, {validator: validateName, trigger: 'blur' }],
        // costPrice:[{required: true, message: '请输入商品成本价'}, {validator: validateCost, trigger: 'blur' }],
        retailPrice:[{required: true, message: '请输入商品零售价'}, {validator: validateCost, trigger: 'blur' }],
        vipPrice:[{validator: validateMemberPrice, trigger: 'blur' }],
        memberPriceType:[{required: true, message: '请选择会员类型'}],
        category: [{required: true, validator: validateCategory, message: '请选择商品分类'}],
        commissionType: [{message: '请选择提成类型'}],
        firstCategoryId: [{required: true, message: '请选择商品分类'}],
        categoryId: [{required: true, message: '请选择商品分类'}],
        serviceCommission: [{required: true, message: '请输入金额'}, {validator: validateServiceCommission, trigger: 'blur' }],
        ...
        commission: [{required: true, message: '请输入提成比例'}, {validator: validatePrice, trigger: 'blur' }]
      ...
      if (this.memberLevelList && this.memberLevelList.length > 0) {
        this.memberLevelList.forEach(item => {
          ...
          if (this.backendPriceType === 'CUSTOMER') {
            ...
            price[item.vipId] = NP.times(NP.divide(item.vipDiscount || 0, 100), this.goods.retailPrice || 0).toFixed(2)
          }
        })
      }
      return [price]
  ...
  inject: ['reload'],
  components: { goodsSpec, addOrUpdate, brandModal, gtSubtitle, gtImageUpload, gtArticleForm, gtProductDetail },
  methods: {
    ...mapMutations({
      saveInfo: 'goods/SAVE'
    }),
        ...
        if (!this.id) {
          ...
          if (item.stock || item.stock == 0) {
            if ( !reg.test(item.stock)) {
              this.$message.warning('规格信息中“商品库存”请填写大于等于0的整数！')
              flag = false
              break
            } else if (item.stock > 99999) {
              this.$message.warning('规格信息中“商品库存”请填写小于等于99999的整数！')
              flag = false
              break
            }
          } else {
    ...
    checkMemberPrice() {
      if (this.goods.priceType === 'MEMBER' && this.goods.memberSwitch) {
        const memberPriceItem = this.memberLevelData && this.memberLevelData.length > 0 ? this.memberLevelData[0] : null
        if (memberPriceItem) {
          ...
          for (var i = 0 ; i < keys.length ; i++) {
            ...
            if (value) {
              if (!priceRange(value, 499999.99)) {
                this.$message.warning('会员价请输入0.01-499999.99的价格数字，最多保留小数点后两位！')
                flag = false
                break
              }
            } else {
    ...
    handleUpdate(createType) {
      const handleSubmit = async() =>{
        ...
        if (this.goods.type === 'ENTITY' || this.goods.type === "BULK") {
          /** 新交互方式处理逻辑 */
          const { barList = [], specItemOne, specItemTwo } = this.$refs['specComp'] || {}
          if (!this.checkSpecs(barList, specItemOne, specItemTwo)) {
            return
          }
          if (barList && barList.length > 0) {
            specInfoList = barList.map(item => {
              let { barcode, pic, stock, specOne, specTwo,newCode } = item
              let specValueIds = []
              if (specOne) {
              ...
              return {
                specValueIds,
                barcode,
                stock:this.goods.type === "BULK" ? stock*1000 : stock,
                newCode,
                pic
              }
            })
        ...
        let pics = this.goods.pics.map(item => {
          return item.url
        }).join(',')
        ...
        let submitData = {
          ...
          pricePlans,
          priceType,
          cover: this.goods.pics && this.goods.pics.length > 0 ? this.goods.pics[0].url : null,
          ...
          costPrice: this.goods.costPrice !== null ? this.priceNum(this.goods.costPrice, 'out') : null,
      ...
      this.$refs['goods-form'].validate(async (valid) => {
        if (valid && this.checkMemberPrice()) {
          ...
          // 服务类商品卡扣技师提成判断
          if (this.goods.type == 'SERVICE' && Number(this.goods.serviceCommission) > Number(this.goods.retailPrice)) {
    ...
    async getGoodsInfo() {
      ...
      if (data.priceType === 'MEMBER') {
        ...
        // let price = {}
        // this.memberLevelList.forEach(item => {
        //   const priceItem = data.pricePlans.find(ele => ele.id === item.vipId)
        //   price[item.vipId] = priceItem ? priceItem.price : null
        // })
        // this.memberLevelData = [price]
      }
      // 商品提成
      const commission = data.commissionPlan ? data.commissionPlan.commission : 0
      const commissionType = data.commissionPlan ? data.commissionPlan.commissionType : null
      const serviceCommission = data.commissionPlan ? data.commissionPlan.technicianVal : 0
    ...
    async getSpecKey () {
      if (this.specCb) {
        ...
        if (data.code == 0 && data.data.records && data.data.records.length > 0) {
