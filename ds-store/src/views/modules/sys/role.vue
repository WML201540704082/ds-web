  ...
  <div class="mod-store mod-role sysItems">
    <el-card>
      <el-form :inline="true" :model="dataForm" @keyup.enter.native="reset('search')">
        ...
        <el-form-item class="search-create">
          <gt-button type="search" v-if="isAuth('sys:role:list')" @click.native="reset('search')"/>
          <gt-button type="reset" @click.native="reset('reset')" />
        </el-form-item>
      </el-form>
    </el-card>
    <el-card class="mode-store-table">
      <gt-border-button class="gt-table-create" v-if="isAuth('sys:role:add')" type="create" @click.native="addOrUpdateHandle()" style="margin-bottom: 10px;"/>
      <el-table
        v-table-height
        :data="dataList"
        border
