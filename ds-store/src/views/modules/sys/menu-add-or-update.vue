<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" label-width="100px">
      ...
      <el-form-item label="上级菜单" prop="parentName" v-if="isAuth('sys:menu:select')">
        <el-popover
          ref="menuListPopover"
...
export default {
  ...
  methods: {
    init (id) {
      ...
      this.$http({
        url: this.$http.adornUrl('/sys/store/sysMenu/select'),
        method: 'get',
    ...
    dataFormSubmit () {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.isSave= true
          this.$http({
            url: this.$http.adornUrl(`/sys/store/sysMenu/${!this.dataForm.id ? 'add' : 'update'}`),
            method: 'post',
            selfHandleError: true,
