  ...
  <section class="addactive addactive1"
   v-loading.fullscreen.lock="loading"
    ...
    <el-form :model="activeadd" ref="activeadd" :rules="activeaddaddRules" label-width="110px">
      ...
      <div class="rights">
        ...
        <div v-show="!isHidden">
          ...
          <el-card>
          <el-form-item v-if="this.id" label="审核说明：" prop="checkContent">
             <el-input  type="textarea" v-model="activeadd.checkContent" style="width: 300px;" :disabled=true :maxlength="50" placeholder="最多支持50个字符" autosize></el-input>
          </el-form-item>
        </el-card>
        </div>
      </div>
    ...
    <div class="gt-form-bottom-submit" v-if="!isHidden">
      ...
      <gt-button size="large" v-if="type!='detail'" @click.native="submitCustomer(false)" :loading="$store.state.loading.global" txt="暂存"/>
      <gt-button size="large" v-if="saveTitle" @click.native="submitCustomer(true)" :loading="$store.state.loading.global" txt="保存并提交"/>
    </div>
...
export default {
  ...
  components:{ gtSubtitle, gtImageUpload, gtSelectGoods, gtSpecailModel,gtTimeRange,gtWithdrawalAmountSet,gtSelectGallery },
  data(){
  ...
  methods:{
    ...
    async submitCustomer(submit){
      // console.log(this.activeadd)
      if(!this.checkSetName()){
        this.activeadd.commissionRuleList.map(o=>{
          if(validatenull(o.normal)){
            o.normal = 0
          }
        })
        this.isprice('out')
        console.log({
          ...
          id: this.id,
          submit: submit,
          createOrUpdate: this.id?((this.saveTitle&&this.type==='detail')?false:true):true
        })
        const {data} = await saveGroupBuy({
          ...this.activeadd,
          submit: submit,
          createOrUpdate: this.id?((this.saveTitle&&this.type==='detail')?false:true):true
        })
