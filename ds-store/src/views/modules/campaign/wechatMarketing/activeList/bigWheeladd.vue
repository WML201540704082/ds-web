  ...
  <section class='bigwheel'
   v-loading.fullscreen.lock="loading"
      ...
      <div class="rights">
        ...
        <el-card>
          <el-form-item v-if="this.id" label="审核说明：" prop="checkContent">
             <el-input  type="textarea" v-model="bigwheel.checkContent" style="width: 300px;" :disabled=true :maxlength="50" placeholder="最多支持50个字符" autosize></el-input>
          </el-form-item>
        </el-card>
      </div>
    ...
    <div class="gt-form-bottom-submit">
      <gt-button size="large" is-default @click.native="$router.go(-1)" txt="取消" v-if="saveTitle" />
      <gt-button size="large" @click.native="flagApply(false)" :loading="$store.state.loading.global" v-if="isType!='detail'" txt="暂存"/>
      <gt-button size="large" @click.native="flagApply(true)" :loading="$store.state.loading.global" v-if="saveTitle" txt="保存并提交"/>
    </div>
...
export default {
  ...
  methods:{
    ...
    submitCustomer(submit){
      // 适用门店--
      this.storeidsget()
      this.$refs['bigwheel'].validate((valid) => {
        if (valid) {
          if(this.allNum != 100){
            this.$message.warning('中奖率必须要等于100%')
            return false
          }
          this.getPrizeName('edit')
          if(this.bigwheel.radio == 2){
            if(this.changeVal(this.bigwheel.share,'limit')){
              this.getParam(submit)
            }
          }else{
            this.getParam(submit)
          }
