  ...
  <div class="mod-store particulars">
    ...
    <el-card class="mode-store-table">
      ...
      <div style="float:right;margin-right:10px;margin-top:5px">
        ...
        <span v-if="storeType == 'NEW_RETAIL_AND_DRINK' || storeType == 'NEW_RETAIL'">散称类商品出入库数量合计：{{details.weight || 0}}kg</span>
        <span v-if="storeRoleName === '店长' && hasAuthorization">成本总价合计：{{priceNum(details.totalPrice) || '--'}}元</span>
      </div>
      <el-table
        :data="dataList"
        border
        ...
        <el-table-column
          ...
          label="出入库类型">
          <template slot-scope="scope">
            <span>{{filterStatus(scope.row.outgoingAndWarehousingType, outInType)}}</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="currentUnit"
...
import { outInType } from '@/utils/dict'
...
export default {
  ...
  methods:{
    ...
    //监听出入库类型名称
    changeInOutType(val){
      const inOutTypeName = this.outInType.find(ele => ele.value === val)
      if (inOutTypeName) {
    ...
    async exportExcel() {
      let criteria = {
        ...this.particulars,
        hide: !(this.storeRoleName === '店长' && this.hasAuthorization),
