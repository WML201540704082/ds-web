<template>
  <section class="page-desk">
    <el-card class="card" v-if="!(newStore == 'true' && companyRoleName == '销管') && !(newStore == 'true' && companyRoleName == '仓库') ">
      <div class="item-title">常用功能</div>
      <el-row v-if="newStore == 'true'&& companyRoleName =='数据运维（技术服务）'">
        <el-col v-show="isAuthMenu(item.isauth)" :span="6" :key="item.id" v-for="item in goListOne">
          <div class="item-container first-line" @click="goListQuery(item.router, item.query)">
            <icon-svg :name="item.icon" class="item-icon theme-color"></icon-svg>
            {{item.name}}
          </div>
        </el-col>
      </el-row>
      <el-row v-if="newStore=='false'">
        <el-col v-show="isAuthMenu(item.isauth)" :span="6" :key="item.id" v-for="item in goListOne">
          <div class="item-container first-line" @click="goListQuery(item.router, item.query)">
            <icon-svg :name="item.icon" class="item-icon theme-color"></icon-svg>
            {{item.name}}
      ...
      <el-row v-if="newStore == 'true'&& companyRoleName !='数据运维（技术服务）'">
        <el-col v-show="isAuthMenu(item.isauth)" :span="6" :key="item.id" v-for="item in newgoListOne">
          <div class="item-container first-line" @click="goListQuery(item.router, item.query)">
            <icon-svg :name="item.icon" class="item-icon theme-color"></icon-svg>
            {{item.name}}
          </div>
    ...
    <el-card class="card" v-if="!(newStore == 'true' && companyRoleName == '销管') && !(newStore == 'true' && companyRoleName == '仓库')">
      <div class="item-title max-title">
        ...
        <el-form class="filter-container">
          <!-- <img class="tip-icon" src="static/img/campaign/wenhao.png" @click="prompt_message('maintain')" alt="提示" /> -->
      ...
      <el-row class="maintain" v-loading="turnoverLoading">
        ...
        <el-col class="item-info"><div class="item-value theme-color">{{priceNum(turnoverData.profit) || 0}}</div><div class="item-label">毛利(元)</div></el-col>
        ...
        <el-col class="item-info" style="border-right: 0;">
          <el-tooltip placement="top">
            ...
            <div class="item-value theme-color">{{priceNum(turnoverData.revenueAmount - turnoverData.expendAmount) || 0}}</div>
          </el-tooltip>
      ...
      <section v-loading="chartLoading">
            ...
            <div class="chart-border rank-container">
              <div class="salesRank">
                ...
                <div class="sales-list" v-if="skusDesc && skusDesc.length > 0">
                  <gt-sku-bar :total="countTotal" :price="item.netSale" :infoleft="`${item.productName} ${item.barcode || ''} ${item.spec || '默认规格' } ${item.category1Name ? item.category1Name + '>' + item.category2Name: ''}`" :count="item.netCount" :index="index" :key="index" v-for="(item, index) in skusDesc" />
                </div>
            ...
            <div class="chart-border rank-container">
              <div class="salesRank">
                ...
                <div class="sales-list" v-if="skusAsc && skusAsc.length > 0">
                  <gt-sku-bar :total="countTotal" :price="item.netSale" :infoleft="`${item.productName} ${item.barcode || ''} ${item.spec || '默认规格'} ${item.category1Name ? item.category1Name + '>' + item.category2Name: ''}`" :count="item.netCount" :index="index" :key="index" v-for="(item, index) in skusAsc" />
                </div>
                <div class="list-empty" v-else>暂无数据</div>
              </div>
            </div>
        ...
        <el-row style="margin:  30px 0 15px;">
          <el-col :span="8" style="padding-right: 10px;">
            <div class="sub-title">
              门店销售额
              <el-tooltip placement="top" content="在统计时间内，各门店的销售额排名">
                <img class="tip-icon" src="static/img/campaign/wenhao.png" alt="" />
              </el-tooltip>
            </div>
            <div class="chart-border store-container list-container">
              <div v-if="orderDatas && orderDatas.length > 0">
                <div class="store-item" :key="item.id" v-for="item in orderDatas">
                  <div class="store-info"><span>{{item.storeName}}</span><span class="right theme-color">￥{{priceNum(item.turnover)}}</span></div>
                  <div class="store-owner">店长：{{item.manager}}/{{item.phone}}</div>
                </div>
              </div>
              <div class="list-empty" v-else>暂无数据</div>
            </div>
          ...
          <el-col :span="8" style="padding: 0 5px;">
            <div class="sub-title">
              顾客数量
              <span class="theme-color">{{customerData.all}}</span>
              <el-tooltip placement="top">
                <div slot="content">1.截至统计时间末，企业及各门店的顾客总数<br/>2.统计时间内，各门店的新增顾客数</div>
                <img class="tip-icon" src="static/img/campaign/wenhao.png" alt="" />
              </el-tooltip>
            </div>
            <div class="chart-border list-container">
              <div v-if="customerData.items && customerData.items.length > 0">
                <gt-process :infoleft="store.storeName" :key="store.id" unit="新增/总数" :value="store.addCount" :total="store.total" v-for="store in customerData.items" />
              </div>
              <div class="list-empty" v-else>暂无数据</div>
            </div>
          </el-col>
    ...
    <el-card v-if="newStore == 'true' && companyRoleName == '销管'">
      <img style="width:88%;height:88%" src="static/img/desk/welcome_xiaoguan.jpg" alt="">
    </el-card>
    <el-card v-if="newStore == 'true' && companyRoleName == '仓库'">
      <img style="width:88%;height:88%" src="static/img/desk/welcome_cangku.jpg" alt="">
    </el-card>
  </section>
</template>
<script>
...
export default {
  data() {
    return {
      ...
      categoryPie: null,
      goListOne: [
        {id:1,icon: 'desk-stock',isauth:'goods',router:'stockManagement-instock-inStockAdd',name:'新增入库单'},
        {id:2,icon: 'desk-goods',isauth:'goods',router:'goods-add-or-update', query: {type: 'company'}, name:'新增商品'},
        {id:3,icon: 'desk-coupon',isauth:'campaign_coupon',router:'campaign-add',name:'新建优惠券'},
        {id:4,icon: 'desk-activity',isauth:'campaign_coupon',router:'campaign-wechatMarketing-activeList-activeList',name:'活动列表'}
      ],
      newgoListOne: [
        {id:1,icon: 'desk-stock',isauth:'goods',router:'stockManagement-instock-inStockAdd',name:'新增入库单'},
        {id:4,icon: 'desk-activity',isauth:'campaign_coupon',router:'campaign-wechatMarketing-activeList-activeList',name:'活动列表'}
      ...
      goListTwo: [
        {id:1,isauth:'goods',router:'stockManagement-stockManagement',name:'库存预警SKU', value: 0,params:{showStockWarn:'YES'}},
        {id:2,isauth:'order',router:'stockManagement-requiredInventory', params: { status: 'CHECK_PENDING'},name:'待审核请货单', value: 0},
        {id:3,isauth:'campaign_coupon',router:'settings-store',name:'即将到期门店', value: 0,params:{soonExpire:15}},
      ]
    }
  },
  components: {
  ...
  mounted() {
    ...
    window.addEventListener('resize', () =>{
      this.saleHistory && this.saleHistory.resize()
      this.categorySale && this.categorySale.resize()
      this.categoryPie && this.categoryPie.resize()
    })
  },
  ...
  methods: {
    ...
    async remind() {
      ...
      if (data.code === 0) {
        this.goListTwo[0].value = data.data.stockWarn
        this.goListTwo[1].value = data.data.reqStock
        this.goListTwo[2].value = data.data.storeSoonExpire
      } else {
  ...
  watch: {
    documentWidth(val) {
      if (this.saleHistory && this.categorySale && this.categoryPie) {
