...
<el-dialog
  ...
  <el-form :model="dataForm" :rules="dataRule" ref="dataForm" label-width="100px">
    <el-form-item label="品牌名称" prop="name" class="gt-input-tip">
      <el-input :maxlength="15" v-model="dataForm.name" placeholder="请输入品牌名称"><span slot="prefix" style="left: 210px;">如：聪尔壮</span></el-input>
    </el-form-item>
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate(async (valid) => {
        if (valid) {
          ...
          let result = null
          if (this.dataForm.id) {
            result = await api.updateBrand(this.dataForm)
          } else {
            result = await api.addBrand({
