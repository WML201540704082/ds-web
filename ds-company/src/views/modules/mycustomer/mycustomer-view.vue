    ...
    <el-form :model="customerInfo" ref="customerInfo" label-width="100px" class="viewForm" >
      ...
      <el-card style="margin-bottom: 10px;">
        ...
        <el-form-item label="注册来源">
          <span class="itemInput">{{customerInfo.source || '--'}}</span>
        </el-form-item>
        <el-form-item label="消费门店" v-if="customerInfo.consumeStoreNameList&&customerInfo.consumeStoreNameList.length>0">
          <span class="itemInput">{{customerInfo.consumeStoreNameList.join(',') || '--'}}</span>
        </el-form-item>
        <el-form-item label="归属员工">
          <span class="itemInput">{{`${customerInfo.storeName || '--'} - ${customerInfo.employee || '--'}`}}</span>
        </el-form-item>
        ...
        <!-- <el-form-item label="储值余额">
          <span class="itemInput">{{priceNum(customerInfo.cardAmount)}}元</span>
        </el-form-item> -->
        ...
        <el-form-item label="顾客资产" v-if="isAuth('company:customerCoupon:list')">
          <span class="previewYHJ theme-background" @click="previewYHJ('CZK')">储值卡</span>
          <span class="previewYHJ theme-background" @click="previewYHJ('GWK')">购物卡</span>
...
import { workRecords } from '@/service/workrecords'
export default {
  ...
  mounted(){
    this.$nextTick(function () {
      ...
      if(this.isAuth('company:customer:detail')){
        this.getcustomerDetails(this.customerid)
      }
      if(this.isAuth('customer:store:sysUser:select')){
        this.getClerkList()
      }
    })
  },
  methods:{
    ...
    async getcustomerDetails(customerId){
      if (customerId) {
        ...
        this.getClerkList(data.employeeId)
        // 回访小计
    ...
    // 查看卡劵
    previewYHJ(type){
      this.$router.push({name:'mycustomer-customerCard',query:{id:this.customerid,name:this.customerInfo.name,phone:this.customerInfo.phone,type:type}})
    }
  }
}
</script>
