   ...
   v-loading.fullscreen.lock="loading"
    ...
    <el-form :model="customeradd" ref="customeradd" :rules="customeraddaddRules" label-width="100px">
      ...
      <el-card style="margin-bottom: 10px;">
        <div>
          ...
          <el-form-item label="归属员工" prop="employeeId">
            <el-select v-model="customeradd.storeId" placeholder="请选择门店"  @change="changeStore">
          ...
          <el-form-item label="积分" prop="integral" v-if="!customerid">
            <el-input v-model.number="customeradd.integral" placeholder="请输入积分值" maxlength="8" style="display: inline;"></el-input>
            <span>(只能填写整数)</span>
          </el-form-item>
      ...
      <el-card class="elCard">
        ...
        <!-- 新增宝宝标签 -->
        <div class="gt-form-bottom-submit" style="margin-top: 0!important" v-if='customeradd.babyList&&customeradd.babyList.length >= 0&&customeradd.babyList.length <= 2'>
          <el-button class="addBaby theme-nextb theme-nexborder theme-color" @click="babyadds">
            ...
            <span class="theme-color">添加一个宝宝</span>
          </el-button>
        </div>
      </el-card>
      ...
      <el-card class="elCard">
        ...
        <div class="gt-form-bottom-submit" style="margin-top: 0!important;" v-if='customeradd.addressList.length >= 0&&customeradd.addressList.length <= 1'>
...
<script>
import { sex, babyFeedMode, babysex} from '@/utils/dict'
...
export default {
  ...
  methods:{
    ...
    changeStore(val){
      this.storeList.map(o=>{
        if(o.id == val){
          this.customeradd.storeName = o.name
        }
      })
      this.customeradd.employeeId = ''
      this.customeradd.storeId = val
      if(val){
        this.getClerkList(val)
      }else{
        this.clerkList = []
      }
    },
    // 获取店员列表
    async getClerkList(storeId,type){
    ...
    async getcustomerDetails(customerId){
      if (customerId) {
        ...
        this.getClerkList(data.storeId,'detail')
      }
    },
    ...
    submitCustomer(){
      ...
      if (this.customeradd.storeId && !this.customeradd.employeeId) {
        this.$message.warning('请选择归属员工！')
        return
      }
