  ...
  <div class="mod-store buy-sale-base">
    <el-card>
      <el-form :inline="true" class="mode-form-line1" label-width="80px" ref="bsb" :model="bsb" @keyup.enter.native="resetForm('search')">
        <el-form-item label="仓库" prop="warehouse">
          <el-select v-model="bsb.warehouse" @change="changeTable(true)" placeholder="请选择销售门店">
    ...
    <el-card class="mode-store-table">
      ...
      <el-table
        ...
        <el-table-column label="供应商采购" v-if="!bsb.warehouse">
          <el-table-column
            align="center"
            min-width="100"
            :show-overflow-tooltip="true"
          ...
          <el-table-column
            ...
            label="采购退货">
            <template slot-scope="scope">
              <span class="amountTop">{{scope.row.purchaseOutWarehouseQuantity}}</span>
              <span class="moneyBottom">￥{{priceNum(scope.row.purchaseOutWarehousePrice).toFixed(2)}}</span>
            </template>
          </el-table-column>
        </el-table-column>
        <el-table-column label="采购/总部配送" v-if="bsb.warehouse">
          <el-table-column
            prop="orderId"
            align="center"
            min-width="100"
        ...
        <el-table-column label="销售" v-if="!bsb.warehouse">
          <el-table-column
            align="center"
            min-width="100"
            :show-overflow-tooltip="true"
            label="批发团购销售">
            <template slot-scope="scope">
              <span class="amountTop">{{scope.row.wholesaleGroupPurchaseSalesQuantity}}</span>
              <span class="moneyBottom">￥{{priceNum(scope.row.wholesaleGroupPurchaseSalesPrice).toFixed(2)}}</span>
            </template>
          </el-table-column>
        </el-table-column>
        <el-table-column label="销售" v-if="bsb.warehouse">
        ...
        <el-table-column label="门店配送" v-if="!bsb.warehouse">
          <el-table-column
            align="center"
            min-width="100"
            :show-overflow-tooltip="true"
            label="配送出库">
            <template slot-scope="scope">
              <span class="amountTop">{{scope.row.deliveryOutQuantity}}</span>
              <span class="moneyBottom">￥{{priceNum(scope.row.deliveryOutPrice).toFixed(2)}}</span>
            </template>
          </el-table-column>
          <el-table-column
            :show-overflow-tooltip="true"
        ...
        <el-table-column label="门店调拨" v-if="bsb.warehouse">
          <el-table-column
            align="center"
            min-width="100"
            :show-overflow-tooltip="true"
        ...
        <el-table-column label="寄存" v-if="bsb.warehouse">
          <el-table-column
            align="center"
            min-width="100"
            :show-overflow-tooltip="true"
        ...
        <el-table-column label="其他" v-if="!bsb.warehouse">
          <el-table-column
            align="center"
            min-width="100"
            :show-overflow-tooltip="true"
  ...
  methods:{
    ...
    async getDataList(flag){
      ...
      const { data } = await getPurchasePageList({
        ...
        ...this.bsb,
        warehouse:this.bsb.warehouse ? this.bsb.warehouse : 0,
      })
      ...
      if (data && data.code === 0) {
        this.dataList = data.data.records.map(item=>{
          return{
            ...
            purchaseOutWarehouseQuantity:item.productType == 'BULK' ? item.purchaseOutWarehouseQuantity / 1000 : item.purchaseOutWarehouseQuantity,//采购退货
            ...
            wholesaleGroupPurchaseSalesQuantity:item.productType == 'BULK' ? item.wholesaleGroupPurchaseSalesQuantity / 1000 : item.wholesaleGroupPurchaseSalesQuantity,//批发团购销售
            ...
            deliveryOutQuantity:item.productType == 'BULK' ? item.deliveryOutQuantity / 1000 : item.deliveryOutQuantity,//配送出库
