  ...
  <div class="mod-store customers">
    ...
    <el-card class="mode-store-table">
      ...
      <div style="float:right;margin-right:10px;margin-top:5px">
        <span>零售类退库数量合计：{{details.sumRetailNum || 0}}</span>
        <span>散称类退库数量合计：{{details.sumBulkNum || 0}}kg</span>
        <span>退库金额合计：{{details.sumProductCostPriceAmount || 0}}元</span>
        <span>成本金额合计：{{details.sumCostPriceAmount || 0}}元</span>
      </div>
      <el-table
        :data="dataList"
        border
...
import { getMainCategory, getSubCategory ,exportExcel} from "@/service/goods";
...
export default {
  name: "statement-returnGoodsDetails-returnGoodsDetails",
  data() {
    return {
      storeList:[],
      // supplierList:[],
  ...
  methods: {
    ...
    async getDataList() {
      ...
      const { data } = await api.getReturnStockDetailList({
        size: this.pageSize,
      ...
      if(data && data.code ==0){
        ...
        if( data.data.page.records){
             ...
             return{
              ...item,
              // returnStockAmount:this.priceNum(item.returnStockAmount),//退库金额
    ...
    async changeSecondCatetory(id) {
      ...
      const criteria = {
       ...
       this.exportList(this.total, '退库商品明细列表', 'REPORT_RETURN_STOCK_ITEMS', criteria)
    },
    // 分页
