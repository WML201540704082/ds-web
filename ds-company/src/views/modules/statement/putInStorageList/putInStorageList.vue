  ...
  <div class="mod-store customers">
    ...
    <el-card class="mode-store-table">
      ...
      <div style="float:right;margin-right:10px;margin-top:5px">
        <span>入库金额合计：{{details.sumCostPriceAmount || 0}}</span>
      </div>
      <el-table
        :data="dataList"
        border
...
export default {
  name: "statement-putInStorageList-putInStorageList",
  data() {
    return {
      storeList: [],
      filterStatus,
 ...
 inject: ['exportList'],
  ...
  methods: {
    ...
    async getDataList() {
      ...
      const { data } = await api.getPutInStoageList({
        size: this.pageSize,
      ...
      if(data && data.code ==0){
        if(data.data.page.records){
            this.dataList = data.data.page.records.map(item=>{
              return{
              ...item,
            // costPriceAmount: this.priceNum(item.costPriceAmount),//入库金额
              }
          })
        }else{
          this.dataList = []
    ...
    async exportExcel() {
      ...
      this.exportList(this.total, '入库单列表', 'REPORT_IN_STOCK_ORDER', criteria)
    },
