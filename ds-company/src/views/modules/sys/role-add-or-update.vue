...
export default {
  ...
  methods: {
    ...
    async getsys(){
      const originalMenu = await this.selects()
      this.visible = true
      this.$nextTick(() => {
    ...
    async detail(originalMenu){
      if (this.dataForm.id) {
        const data = await this.$http({
          url: this.$http.adornUrl(`/sys/company/sysRole/detail/${this.dataForm.id}`),
          method: 'get',
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.isSave = true
          this.$http({
            url: this.$http.adornUrl(`/sys/company/sysRole/${!this.dataForm.id ? 'add' : 'update'}`),
            method: 'post',
