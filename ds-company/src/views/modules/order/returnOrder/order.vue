<template>
  <div class="mod-store customers">
    <el-card>
      <gt-advance ref="advance" :orderName="'return'" @refreshSearch="getList"/>
    </el-card>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <!-- <span v-if="scope.row.stage === 'WAIT_REFUND'">|</span>
            <el-button v-if="scope.row.stage === 'WAIT_REFUND'" class="nopadding" type="text" size="small" @click="refundReview(scope.row.id)">退款审核</el-button> -->
            <!-- <span v-if="scope.row.stage === 'APPROVE'">|</span>
            <el-button v-if="scope.row.stage === 'APPROVE'" class="nopadding" type="text" size="small" @click="refund(scope.row.id)">退款</el-button> -->
          </template>
        </el-table-column>
        <el-table-column
          prop="id"
  ...
  methods: {
    ...
    async getDataList() {
      ...
      const { data } = await api.getreturnOrderList({
        ...
        ...this.order,
        guideId:this.order.guideId == '' ? null : this.order.guideId
      })
    ...
    refund(id) {
      this.$easyConfirm.show({
        ...
        content: '您确定要进行退款操作吗？',
        confirm: async () => {
          const { data } = await api.returnOrderrefund({
            comment: '',
            id: id
          })
          if (data.code === 0) {
            this.$easyConfirm.hide()
            this.$message.success('已发出退款申请')
            this.getDataList()
          } else {
