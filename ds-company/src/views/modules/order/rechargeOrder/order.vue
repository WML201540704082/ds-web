<template>
  <div class="mod-store customers">
    <el-card>
      <gt-advance ref="advance" :orderName="'recharge'" @refreshSearch="getList"/>
    </el-card>
    ...
    <el-card class="mode-store-table">
      ...
      <el-table
        ...
        <el-table-column
          ...
          label="充值金额">
          <template slot-scope="scope">
            <span>{{number(priceNum(scope.row.payAmount))}}</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="giftAmount"
          ...
          label="赠送金额">
          <template slot-scope="scope">
            <span>{{number(priceNum(scope.row.giftAmount))}}</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="payAmount"
...
import { paymentOfMode,rechargeProject, orderType, retailOrder } from '@/utils/dict'
import { filterStatus, filterColor, number, fileDownload } from '@/utils'
import * as api from '@/service/order'
import moment from 'moment'
export default {
  name: 'order-rechargeOrder-order',
  data () {
    return {
      filterStatus,
      filterColor,
  ...
  beforeRouteEnter (to, from, next) {
    next(vm =>{
      if(from.name !== 'rechargeOrder-view'){
        vm.reload()
      }
    })
  },
  ...
  methods: {
    ...
    async getDataList() {
      ...
      const { data } = await api.getrechargeOrderList({
        size: this.pageSize,
    ...
    async exportExcel() {
      const criteria = {
        ...
        cardType: this.order.cardType || null,
        shoppingCardId: this.order.shoppingCardId || null,
        type: this.order.type || null,
        stage: this.order.stage || null,
      }
      this.exportList(this.total, '充值订单列表', 'RECHARGE_ORDER', criteria, async () => {
        const { data, headers } = await api.rechargeexport({
          ...criteria,
          size: 10000,
          page: 0
        })
        fileDownload(data, '充值订单列表.xls')
      })
    },
    // 查看
    storeViewHandle (id){
      this.$router.push({name:'rechargeOrder-view',query:{id}})
    },
    // 分页
