<template>
  <div class="lefts">
    <div class="phoneTop">
      <img src="static/img/goods/phone/meng_zhuan_title.png" alt="">
      <span>{{activeadd.title}}</span>
    </div>
    <div class="phoneCenter">
      <img style="width:100%" src="static/img/goods/phone/meng_zhuan_title-min.png" alt="">
      ...
      <div class="TurntablePage" v-if="activeadd.items&&activeadd.items.length>0">
        <div class="TurntableBox">
          <div class="turnplate">
            ...
            <img class="pointer" @click="pointerClick" src="static/img/goods/phone/start.png">
          </div>
        </div>
      </div>

      <img style="width:100%;margin: 10px 0" src="static/img/goods/phone/meng_zhuan_button.png" alt="">
      <div class="clickTab">点击转盘开始抽奖</div>
    </div>
    <!-- 抽奖说明 -->
    <div class="prizeInfo">
      <img style="width:80%" src="static/img/goods/phone/meng_zhuan_title1-min.png" alt="">
    </div>
    <div class="prizeContent">
      <!-- 奖品说明 -->
      <div class="backgrounds">
        <div class="expand">
          <img src="static/img/goods/phone/meng_zhuan_point-min.png" alt="">
          <span>奖品说明</span>
        </div>
        <div class="gifts colorCommon" v-for="(item,index) in activeadd.items" :key="index">{{index+1}}等奖：{{item.name}}</div>
      </div>
      ...
      <!-- <div class="backgrounds backgrounds1" v-if="activeadd.storeList&&activeadd.storeList.length>0">
        ...
        <div class="stores">
          <div class="storelist" v-for="item in activeadd.storeList" :key="item.id">
            ...
            <img src="static/img/goods/phone/meng_zhuan_phone-min.png" alt="">
          </div>
        </div>
      </div> -->
      ...
      <div class="backgrounds">
        ...
        <div class="gifts colorCommon">参与条件：
          ...
          <span v-if="activeadd.radio == 1">抽奖次数不限</span>
          <span class="colorCommon" v-if="activeadd.radio == 2">限抽奖{{activeadd.times}}次</span>
          <span class="colorCommon" v-if="activeadd.radio == 3">每位顾客每天限参加{{activeadd.dayTimes}}次</span>
          <div class="colorCommon leftsTop" v-if="activeadd.radio != 1">成功分享一次活动给微信好友，可额外获赠抽奖{{activeadd.gift}}次</div>
          <div class="colorCommon leftsTop" v-if="activeadd.radio != 1">每位用户最多可获赠抽奖{{activeadd.share}}次</div>
        </div>
    ...
    <div class="prizeInfo" v-if="activeadd.templates&&activeadd.templates.length>0">
      <img style="width:80%" src="static/img/goods/phone/meng_zhuan_title2-min.png" alt="">
    </div>
...
<style lang='scss' scoped>
  .TurntablePage {
    background: url('/static/img/goods/phone/meng_zhuan_border.png') no-repeat;
    width: 290px;
    height: 290px;
