<template>
  <div class="mod-store customers">
      ...
      <el-form :model="filterForm" :inline="true" ref='filterForm' class="mode-form-line1"  @keyup.enter.native="resetForm('search')">
        ...
        <el-form-item label="活动门店" prop='storeId'>
          <el-select v-model="filterForm.storeId" placeholder="请选择活动门店" filterable clearable>
            <el-option  label="全部" value="" ></el-option>
            <el-option
        ...
        <div style="display:none;" class="button_check">
          ...
          <el-form-item label="佣金状态" prop='commissionStatus'>
            <el-select v-model="filterForm.commissionStatus" clearable>
                <el-option label="全部" value="" ></el-option>
                <el-option
                v-for="item in commanderStatus"
                :key="item.value"
                v-show="item.value!='NONE'"
                :label="item.label"
                :value="item.value">
                </el-option>
            </el-select>
...
<script>
import { resultStatus,activeType,commanderStatus} from '@/utils/dict'
