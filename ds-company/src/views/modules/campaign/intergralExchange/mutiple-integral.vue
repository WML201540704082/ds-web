<template>
  <section class="page-mutiple-integral-rule">
    <!-- table列表 -->
    <div class="mutiple-title theme-color">会员等级多倍积分：<el-tooltip placement="top">
    ...
    <el-table
      ...
      <el-table-column
        ...
        :label="item.vipName">
        <template slot-scope="scope">
          <span v-show="pageType === 'detail'">{{item.multiple}}</span>
          <span v-show="pageType === 'edit'">
            <el-input class="table-input" v-model="item.multiple"></el-input>
          </span>
        </template>
      </el-table-column>
    </el-table>
    ...
    <div v-if="vipList && vipList.length > 0" class="mutiple-title theme-color">会员指定周期多倍积分：</div>
...
export default {
  ...
  methods: {
    ...
    async getLevel(obj,vipList) {
      ...
      this.vipDatePlans = (obj.vipDatePlans&&obj.vipDatePlans.id)?{
        ...obj.vipDatePlans,
        id: obj.vipDatePlans.id,
