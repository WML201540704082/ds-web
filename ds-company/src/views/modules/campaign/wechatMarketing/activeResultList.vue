  ...
  <div class="mod-store customers page-customer-list">
    ...
    <el-card class="mode-store-table card1">
      <el-row class="resultInfo">
        ...
        <div class="item-label">交易流水</div>
        </el-col>
        <el-col :span="6" class="item-info"><div class="item-value theme-color">{{allCount.orders}}</div>
        <div class="item-label">交易单数</div>
        </el-col>
        <el-col :span="6" class="item-info"><div class="item-value theme-color">{{allCount.click}}</div>
          <div class="item-label">点击</div>
        </el-col>
        <el-col :span="6" class="item-info"><div class="item-value theme-color">{{allCount.conversion}}</div><div class="item-label">转化</div>
        </el-col>
      </el-row>
      <gt-border-button class="gt-table-create" txt="下载查询结果" v-if="isAuth('company:customerCoupon:add')" type="bind" @click.native="downLoad()"/>
      <el-table
        :data="dataList"
        border
        ...
        <el-table-column
          width="120"
          label="操作" v-if="isAuth('company:order:detail')">
          <template slot-scope="scope">
        ...
        <el-table-column
          ...
          label="交易金额">
          <template slot-scope='scope'>
            <span v-if="scope.row.paidAmount">{{priceNum(scope.row.paidAmount)}}</span>
            <span v-else>0</span>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column
          ...
          label="传播关系">
          <template slot-scope="scope">
            <span>{{scope.row.headerName}}/{{scope.row.headerPhone}}</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="headerLevel"
          ...
          label="传播关系级别">
          <template slot-scope="scope">
            <span v-if="scope.row.headerLevel">{{filterStatus(scope.row.headerLevel, commanderLevel)}}</span>
            <span v-else>--</span>
          </template>
        </el-table-column>
        <el-table-column
          ...
          label="返佣金额">
          <template slot-scope="scope">
            <span>{{priceNum(scope.row.commission)}}</span>
          </template>
        </el-table-column>
        <el-table-column
          ...
          label="佣金状态">
          <template slot-scope="scope">
            <span>{{filterStatus(scope.row.commissionStatus, commanderStatus)}}</span>
          </template>
        </el-table-column>
      </el-table>
...
import { resultStatus,activeType,orderType,zsType,worth,commanderStatus,commanderLevel } from '@/utils/dict'
...
export default {
  ...
  components: {
    gtPagination, gtDelay, gtActiveAdvance
  },
  created () {
    // 获取运行结果列表
    this.getDataList()
