<template>
  <div class="mod-store page-promote-list" v-loading="dataListLoading">
      ...
      <el-card class="mode-store-table">
        <gt-border-button v-if="newStore == 'true'&& companyRoleName =='数据运维（技术服务）'" class="gt-table-create" type="create" txt="新建组合搭售活动" @click.native="() => addOrUpdateHandle()"/>
        <gt-border-button v-if="newStore == 'false'" class="gt-table-create" type="create" txt="新建组合搭售活动" @click.native="() => addOrUpdateHandle()"/>
...
export default {
  ...
  methods: {
    ...
    addOrUpdateHandle (id,status) {
      if (id) {
        this.$router.push({name:'sales-add-or-update', query: {id:id,status:status}})
      } else {
        this.$router.push({name:'sales-add-or-update', query: {id: ''}})
      }
    },
    // 删除
    deleteHandle (id) {
    ...
    storeViewHandle (id){
      if (id) {
        this.$router.push({name:'promote-view', query: {id}})
      } else {
