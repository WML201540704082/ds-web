    ...
    <el-card>
      <el-form label-width="150px">
        ...
        <el-form-item label="适用门店">
          <span class="itemInput">{{coupon.applyToStores && coupon.applyToStores.length > 0 ? coupon.applyToStores.map(item => item.name).join(',') : '全部门店'}}</span>
        </el-form-item>
        <el-form-item label="适用范围">
          <div v-if="coupon.applyToProducts && coupon.applyToProducts.length > 0" class="table-container">
            <el-table
              ...
              <el-table-column
                ...
                label="成本价">
                <template slot-scope="scope">
                  <span v-if="scope.row.costPrice !== null">{{number(scope.row.costPrice/100)}}</span>
                  <span v-else>--</span>
                </template>
              </el-table-column>
              <el-table-column
          ...
          <div class="itemInput" v-else>全店商品</div>
        </el-form-item>
