...
export function commissionRange(price, range = 99999.99,type) {
  var re = /(^[1-9]\d*(\.\d{1,2})?$)|(^0(\.\d{1,2})?$)/
  var price = price>0?Math.abs(price):price
  var range = range>0?Math.abs(range):range
  if (!isNaN(price) && re.test(price)) {
    if(type=='normal'){
      if (price >= 0 && price <= range) {
        return true
      } else {
        return false
      }
 ...
 * limit 限制字数的数量
 */
export function limitInputName (value, limit,type) {
