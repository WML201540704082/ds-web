    ...
    <el-card style="margin-bottom: 10px;padding-bottom: 10px;">
      ...
      <el-table
        ...
        <el-table-column
          ...
          label="商品类别">
          <template slot-scope="scope">
            {{scope.row.type ? filterStatus(scope.row.type,productType) : '--'}}
          </template>
        </el-table-column>
        <el-table-column
          prop="ownership"
...
import { paymentOfMode,orderOfMode, orderType,productType,retailOrder, productOwner, offerType } from '@/utils/dict'
  ...
  async created() {
    ...
    if (id) {
      ...
      if(data&&data.code==0){
        ...
        if (data.data.offers && data.data.offers.length > 1) {
          offers = data.data.offers.sort(function(a, b){ return a.type.charCodeAt() - b.type.charCodeAt()})
        }
        this.baseInfo = {
          ...data.data,
          offers,
          orderShip: data.data.orderShip ? data.data.orderShip : {},
          returnOrder: data.data.returnOrder ? data.data.returnOrder : {},
          createTime: moment(data.data.createTime).format('YYYY-MM-DD HH:mm:ss')
        }
      }else{
