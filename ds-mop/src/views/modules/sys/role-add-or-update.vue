...
export default {
  ...
  methods: {
    ...
    async getsys(){
      const originalMenu = await this.selectMenu()
      this.visible = true
      this.$nextTick(() => {
    ...
    async detail(originalMenu){
      if (this.dataForm.id) {
        const { data } = await this.$http({
          url: this.$http.adornUrl(`/sys/mop/sysRole/detail/${this.dataForm.id}`),
          method: 'get',
        ...
        if (data && data.code === 0) {
          ...
          // }
          let newMeunIds = data.data.menuIdList
          // 主菜单id
          ...
          let newIds = data.data.menuIdList.filter(id => {
    ...
    async selectMenu(){
      const { data } = await this.$http({
        url: this.$http.adornUrl('/sys/mop/sysMenu/list'),
        method: 'get',
        params: this.$http.adornParams()
      })
      let dataList = []
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.isSave = true
          this.$http({
            url: this.$http.adornUrl(`/sys/mop/sysRole/${!this.dataForm.id ? 'add' : 'update'}`),
            method: 'post',
