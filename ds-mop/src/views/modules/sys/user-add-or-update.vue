  ...
  <el-form :model="dataForm" :rules="dataRule" ref="dataForm" label-width="100px">
    ...
    <el-form-item label="角色" size="mini" prop="roleIdList"  v-if="isAuth('sys:role:select')">
      <!-- <el-checkbox-group v-model="dataForm.roleIdList">
        <el-checkbox v-for="role in roleList" :key="role.id" :label="role.id" style="margin-right: 30px;margin-left: 0!important">{{ role.name }}</el-checkbox>
      </el-checkbox-group> -->
...
export default {
  ...
  methods: {
    init (id) {
      ...
      if(this.isAuth('sys:user:detail')){
        this.detail()
      }
    },
    detail(){
      if (this.dataForm.id) {
        this.isWrite = true
        this.$http({
          url: this.$http.adornUrl(`/sys/mop/sysUser/detail/${this.dataForm.id}`),
          method: 'get',
          ...
          if (data && data.code === 0) {
            ...
            // this.dataForm.roleIdList = data.data.mopSysRoleIdList || []
            if(data.data.mopSysRoleIdList){
              this.dataForm.roleIdList = parseInt(data.data.mopSysRoleIdList[0])
            }
    ...
    selectRole(){
      this.$http({
        url: this.$http.adornUrl('/sys/mop/sysRole/select'),
        method: 'get',
        params: this.$http.adornParams()
      }).then(({data}) => {
        if(data && data.code === 0) {
    ...
    dataFormSubmit () {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.isSave = true
          this.$http({
            url: this.$http.adornUrl(`/sys/mop/sysUser/${!this.dataForm.id ? 'add' : 'update'}`),
            method: 'post',
            data: this.$http.adornData({
              'id': this.dataForm.id || undefined,
              'userName': this.dataForm.userName,
              'password': this.dataForm.password,
              // 'mopSysRoleIdList': this.dataForm.roleIdList
              'mopSysRoleIdList':roleLists
            })
          }).then(({data}) => {
            this.isSave = false
            if (data && data.code === 0) {
